/*
  A simple C program that creates and initializes a server socket. Once
  initialized, the server should accept a client connection, close the
  connection, and then exit. Specify the listening port of the server via the
  command line.
*/
...
int main(int argc, char *argv[]) {
  ...
  // write system call writes back to the socket, which the client sees.
  return_value = write(client_socket_fd,"Ack, message received\n", 22);
  if (return_value < 0) {
